...
app.post("/register" , async (request,response)=>{
   ...
   const result1 = await client.db("MainDataBase").collection("userdata").findOne({username:username})
   if(!result1)
   {
    const salt = await bcrypt.genSalt(10)
    const hashpass =await bcrypt.hash(password,salt)
    
    const result = await client.db("MainDataBase").collection("userdata").insertOne({name , email , username , hashpass})
    console.log(result)
...
app.post("/login" , async(request,response)=>{
   ...
   const result = await client.db("MainDataBase").collection("userdata").findOne({username:username})
...
app.get("/auth",validateToken, async(request,response)=>{
    const client = await createconnections()
...
app.put("/patchgame/:id", async(request, response) => {
